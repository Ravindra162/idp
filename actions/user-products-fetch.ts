...
export async function getFilteredProducts(domainId: string, userId: string) {
  ...
  const products = await db.product.findMany({
    ...
    include: {
      includedWalletTypes: true,
      excludedWalletTypes: true,
  ...
  const mergedProducts = filteredProducts.map((product) => {
    const productInfo = productInfos.find(
      (info: { productId: string }) => info.productId === product.id
    );
    ...
    return {
      ...
      visibleToAllDomains: product.visibleToAllDomains,
      visibleToAllWalletTypes: product.visibleToAllWalletTypes,
      includedWalletTypes: product.includedWalletTypes.map((wallet) => ({
        id: wallet.id,
        name: wallet.name,
      })),
      includedWalletTypeIds: product.includedWalletTypeIds,
      excludedWalletTypes: product.excludedWalletTypes.map((wallet) => ({
        id: wallet.id,
        name: wallet.name,
      })),
      excludedWalletTypeIds: product.excludedWalletTypeIds,
      includedDomains: product.includedDomains.map((domain) => ({
        id: domain.id,
        name: domain.name,
      })),
      includedDomainIds: product.includedDomainIds,
      excludedDomains: product.excludedDomains.map((domain) => ({
        id: domain.id,
        name: domain.name,
      })),
      excludedDomainIds: product.excludedDomainIds,
      visibleToAllTeams: product.visibleToAllTeams,
      includedTeams: product.includedTeams.map((team) => ({
        id: team.id,
        name: team.name,
      })),
      includedTeamIds: product.includedTeamIds,
      excludedTeams: product.excludedTeams.map((team) => ({
        id: team.id,
        name: team.name,
      })),
      excludedTeamIds: product.excludedTeamIds,
    };
  });
...
export async function filterProductsByTeam(
  mergedProducts: any[],
  teamId: string
) {
...
export async function filterProductsByWalletType(
  mergedProducts: any[],
  walletTypeId: string
) {
...
export async function filterProductsForProUser(
  ...
  const finalProducts = filteredProducts.map((product) => {
    const proProductInfo = proProductInfos.find(
      (info) => info.productId === product.id
    );
...
export async function getFinalFilteredProducts(
  domainId: string,
  userId: string,
  teamId: string,
  walletTypeId: string
  ...
  try {
    ...
    const domainFilteredProducts =
      (await getFilteredProducts(domainId, userId)) ?? [];

    // Step 2: Filter by Team
    const teamFilteredProducts =
      (await filterProductsByTeam(domainFilteredProducts, teamId)) ?? [];

    // Step 3: Filter by Wallet Type
    const walletFilteredProducts =
      (await filterProductsByWalletType(teamFilteredProducts, walletTypeId)) ??
      [];

    // Step 4: Apply PRO Pricing (if applicable)
    const proFilteredProducts =
      (await filterProductsForProUser(walletFilteredProducts, userId)) ?? [];
