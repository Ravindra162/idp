...
import { addDomain } from "@/actions/add-wallet-type";
import { useRouter } from "next/navigation";

const domainSchema = z.object({
  domainId: z.string().min(1, "Domain ID is required"), // Ensures domainId is a non-empty string
});
...
const AddPanelForm = ({
  ...
  const form = useForm<z.infer<typeof domainSchema>>({
    ...
    defaultValues: {
      domainId: "", // Default value for domainId
    },
  });
  const onSubmit = (values: { domainId: string }) => {
    setError("");
    startTransition(() => {
      addDomain(values.domainId, walletTypeId).then((data) => {
        if (data?.success) {
